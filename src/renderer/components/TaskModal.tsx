...
const TaskModal: React.FC<TaskModalProps> = ({ onClose, initialProject, onCreateTask }) => {
  ...
  useEffect(() => {
    ...
    nameFromBranchRef.current = false;
  ...
  // True when the task name was derived from the custom branch name
  const nameFromBranchRef = useRef(false);
  ...
  const handleBranchNameChange = (val: string) => {
    ...
    // Derive task name from branch when the user hasn't manually typed a task name
    if (!userHasTypedRef.current || nameFromBranchRef.current) {
      const trimmed = val.trim();
      if (trimmed) {
        // Use the last path segment as the task name (e.g. "fix/login-bug" → "login-bug")
        const lastSegment = trimmed.split('/').filter(Boolean).pop() || trimmed;
        const derived = normalizeTaskName(lastSegment);
        setTaskName(derived);
        setError(validate(derived));
        nameFromBranchRef.current = true;
      } else if (nameFromBranchRef.current) {
        // Branch cleared — restore auto-generated name
        nameFromBranchRef.current = false;
        const restored = autoGeneratedName || '';
        setTaskName(restored);
        setError(validate(restored));
      }
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onCreateTask(
        ...
        customBranchName.trim() || undefined
      );
      onClose();
    } catch (error) {
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col">
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-4">
          ...
          {useWorktree && (
            <div>
              <Label htmlFor="custom-branch-name" className="mb-2 block">
              ...
              <Input
                ...
                placeholder="fix/my-feature-branch"
                maxLength={200}
                className="w-full"
              />
